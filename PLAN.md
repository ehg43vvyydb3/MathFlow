...
- **사람 보정은 규칙으로 환원한다**: 검토 UI에서 사용자가 고친 내역을 자동 분석과
  ...
  - 남은 미해결 패턴: (1) "필수 05"/"확인체크 12" 같은 원형 아이콘 안에 번호가 박힌
    배지 — 여전히 못 잡음 (아이콘과 번호가 분리 안 돼 있어 간격 기반 라벨 분리가
    안 통함 — 색상 기반 아이콘 검출이 필요해 보이나 아직 미착수, 23페이지 diff에서
    28건으로 가장 큰 패턴). (2) "더 다양한 문제는 RPM..." 참고문구 — 실제 문제
    내용과 한 블록에 뭉쳐 있어서 통째로 제외하는 규칙은 위험해 보류 (2건뿐, 표본도 적음).
...
- [x] "1. 평면좌표" 단원(10~33쪽) 자동 분석 — 사용자 보정 진행 중 (10~24쪽 완료)
...
- [ ] "1. 평면좌표" 나머지 25~33쪽 보정 (9쪽 남음) → 단원 전체 diff 재분석 →
      아이콘 배지("필수 05" 등 색상 원형 라벨) 포함 남은 패턴 한 번에 정비
      (한 단원 다 끝낸 뒤 모아서 보기로 방침 정함 — 2026-07-11)
